...
import { DeleteAccountSection } from "@/components/account/delete-account-form";
 ...
 * anyway (spec §11.5). The danger zone (issue 18, spec §8.1) lives below
 * the form so deletion is reachable but never adjacent to Save.
 */
export default async function SettingsPage() {
  const session = await getSession();
  if (!session) redirect("/login");
  return (
    <section aria-labelledby="settings-heading" className="grid min-w-0 gap-8">
