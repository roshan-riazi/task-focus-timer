...
- browser-trial: 2026-09-19 — sign-in landed on the public `/` (looks logged-out) because `login-form` pushed `/`; fixed to `/app` per spec §7.1 with unit + e2e coverage. Note: local e2e currently collides with the live trial stack on :3000 (its `APP_URL=localhost` rejects e2e's `127.0.0.1` Origin); final e2e proof via CI.
